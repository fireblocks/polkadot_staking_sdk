...
export class DOTStaker {
  ...
  private async refreshBalance(vaultAccountId: string) {
    ...
    await this.apiClient.refreshVaultAssetBalance(
      vaultAccountId,
      this.getAssetId()
    );
  }
  ...
  private async getAvailableBalance(vaultAccountId: string): Promise<string> {
    return (
      await this.apiClient.getVaultAccountAsset(
        vaultAccountId,
        this.getAssetId()
      )
    ).available;
  ...
  public async bond(
    ...
    await this.sendTransaction({
      ...
      params: [
        "staking.bond",
        (this.testnet
          ? amountToBond * 1000000000000
          : amountToBond * 10000000000
        ).toString(),
  ...
  public async withdrawUnbonded(vaultAccountId: string) {
    await this.sendTransaction({
      params: ["staking.withdrawUnbonded", null],
      vaultAccountId,
      txNote: `Withdrawing Unbonded Funds`,
    });
  }
  ...
  public async nominate(vaultAccountId: string, validators: any[]) {
    await this.sendTransaction({
      params: ["staking.nominate", validators],
      vaultAccountId,
   ...
   *      @param proxyType - the originally assigned proxy type (default is 'NonTransfer')
  ...
  public async callFromProxy(args: CallFromProxy) {
    await this.sendTransaction({
      params: [
        ...
        args.realAddress,
        args.proxyType ? args.proxyType : "NonTransfer",
